    ...
    try {
      ...
      switch (payload.type) {
        case 'escalation.created':
          taskId = await handleEscalationCreated(payload, errorContext);
          break;
        case 'task.created':
        ...
        case 'ai.help_requested':
          taskId = await handleAIHelpRequested(payload, errorContext);
          break;
        default:
      ...
      console.error('Unexpected error processing Conduit webhook:', error);
...
async function handleEscalationCreated(payload: any, errorContext: WebhookErrorContext): Promise<string | undefined> {
  return wrapProcessingOperation(async () => {
    const taskData = await mapConduitEventToTask(payload);

    if (!taskData) {
      throw new WebhookProcessingError(
        'Could not map Conduit escalation to task - missing required fields or invalid data',
        { ...errorContext, operation: 'map_escalation', payload }
      );
    }
...
async function handleTaskCreated(payload: any, errorContext: WebhookErrorContext): Promise<string | undefined> {
  return wrapProcessingOperation(async () => {
    ...
    if (!taskData) {
      throw new WebhookProcessingError(
        'Could not map Conduit task to internal task - missing required fields or invalid data',
...
async function handleTaskUpdated(payload: any, errorContext: WebhookErrorContext): Promise<string | undefined> {
  return wrapProcessingOperation(async () => {
    ...
    if (tasks.length === 0) {
      throw new WebhookProcessingError(
        `No matching task found for Conduit update with sourceId: ${sourceId}`,
...
async function handleAIHelpRequested(payload: any, errorContext: WebhookErrorContext): Promise<string | undefined> {
  return wrapProcessingOperation(async () => {
    const taskData = await mapConduitEventToTask(payload);

    if (!taskData) {
      throw new WebhookProcessingError(
        'Could not map Conduit AI help request to task - missing required fields or invalid data',
        { ...errorContext, operation: 'map_ai_help_request', payload }
      );
    }
